param(
  ...
  [string]$At = "02:30",
  [string]$ReplicaRoot = (Join-Path (Split-Path -Parent $ProjectRoot) "cross-border-ai-backup-replica")
)
...
$action = New-ScheduledTaskAction -Execute $python -Argument "`"$script`" --source `"$database`" --target-dir `"$target`" --replica-dir `"$ReplicaRoot`" --retention 14" -WorkingDirectory $ProjectRoot
...
Write-Output "已安装计划任务 $TaskName，每日 $At 执行；独立副本目录：$ReplicaRoot"
